...
## Frontend (DMZ):
The client-facing interface (built with Angular/Typescript) is hosted on GCP, serving as a Demilitarized Zone (DMZ) to securely isolate user traffic from the core backend infrastructure. Fly.io is used only for rerouting.
## Backend API:
The business logic is handled by a Spring Boot application running on Google Cloud Platform (GCP). It exposes a set of RESTful APIs to manage data and power the site's dynamic features.
## Database:
A PostgreSQL database, managed via the Supabase cloud service, provides a reliable and scalable persistence layer.
...
## Secure, Decoupled Design:
The multi-tier architecture with a proxy layer enhances security and allows each component to be developed, deployed, and scaled independently.
